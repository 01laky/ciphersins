...
| `version`               | `1.2.0`                                | npm pin for `ciphersins`, or `workspace` for monorepo dev                                       |
...
```yaml
- run: npx ciphersins@1.2.0 scan ./src --format sarif --output ciphersins.sarif --fail-on high --no-color
```
