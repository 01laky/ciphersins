...
describe("cross-rule integration", () => {
	it("CS-INT-01 all good fixture directories scan with expected deliberate findings", async () => {
	...
	it("CS-INT-02 all bad fixture directories include all nineteen rule hits", async () => {
		const result = await scan({
			paths: allBadDirs,
			cwd: rootDir,
		});
	...
	it("CS-INT-04 combined bad directories yield exact per-rule finding counts", async () => {
		...
		expect(byRule("CS-JWT-01")).toBe(19);
	...
	it("CS-INT-07 combined bad directories include all nineteen rules", async () => {
		const result = await scan({
			paths: allBadDirs,
			cwd: rootDir,
		});
	...
	it("CS-INT-08 combined bad directories yield exact per-rule finding counts with v1.3 rules", async () => {
		const result = await scan({
			paths: allBadDirs,
			cwd: rootDir,
		});
		...
		expect(byRule("CS-JWT-03")).toBe(28);
		expect(byRule("CS-JWT-04")).toBe(23);
		expect(byRule("CS-JWT-05")).toBe(16);
		expect(byRule("CS-JWT-06")).toBe(3);
		expect(byRule("CS-CMP-01")).toBe(18);
		expect(byRule("CS-RNG-01")).toBe(19);
		expect(byRule("CS-RNG-02")).toBe(4);
		expect(byRule("CS-HASH-01")).toBe(33);
		expect(byRule("CS-HASH-02")).toBe(28);
		expect(byRule("CS-HASH-03")).toBe(7);
		expect(byRule("CS-HASH-04")).toBe(5);
		expect(byRule("CS-HASH-05")).toBe(5);
		expect(byRule("CS-ENC-01")).toBe(15);
		expect(byRule("CS-ENC-02")).toBe(6);
		expect(byRule("CS-ENC-03")).toBe(5);
		expect(byRule("CS-ENC-04")).toBe(4);
	...
	it("CS-INT-09 jwt good fixtures stay clean with nineteen rules active", async () => {
		const result = await scan({ paths: [jwtGoodDir], cwd: rootDir });
	...
	it("CS-INT-11 all nineteen good dirs plus edge-cases yield HASH-02 ENC-01 JWT-05 and RNG-01", async () => {
		const result = await scan({
	...
	it("CS-INT-15 bcrypt-hash-password.ts stays clean with all nineteen rules", async () => {
	...
	it("CS-INT-17 all nineteen good directories scan with expected file counts", async () => {
		const result = await scan({
			paths: allGoodDirs,
			cwd: rootDir,
		});
	...
	it("CS-INT-19 jwt-02 good and jwt-01 bad yields JWT-01 and optional JWT-05 from good", async () => {
		const result = await scan({
		...
		expect(
			result.findings.every(
				(f) => f.ruleId === "CS-JWT-01" || f.ruleId === "CS-JWT-05",
			),
		).toBe(true);
		expect(result.findings.some((f) => f.ruleId === "CS-JWT-01")).toBe(true);
	});
	...
	it("CS-INT-21 jwt-02 good hash-02 good and jwt-02 bad yields JWT-02 HASH-02 and optional JWT-05", async () => {
	...
	it("CS-INT-23 jwt-02 good directory alone scans 24 files with no JWT-02 findings", async () => {
	...
	it("CS-INT-24 verify-shorthand-algorithms.ts stays clean with all nineteen rules", async () => {
	...
	it("CS-INT-25 indirect-verify-ref.ts stays clean with all nineteen rules", async () => {
	...
	it("CS-INT-27 jwt-04 good and jwt-03 bad yields JWT-03 and optional JWT-05", async () => {
		...
		expect(
			result.findings.every(
				(f) => f.ruleId === "CS-JWT-03" || f.ruleId === "CS-JWT-05",
			),
		).toBe(true);
		expect(result.findings.some((f) => f.ruleId === "CS-JWT-03")).toBe(true);
	});

	it("CS-INT-28 jwt-03 good and jwt-04 bad yields JWT-04 and optional JWT-05", async () => {
		...
		expect(
			result.findings.every(
				(f) => f.ruleId === "CS-JWT-04" || f.ruleId === "CS-JWT-05",
			),
		).toBe(true);
		expect(result.findings.some((f) => f.ruleId === "CS-JWT-04")).toBe(true);
	});
	...
	it("CS-INT-30 nineteen good dirs file count exact", async () => {
	...
	it("CS-INT-35 jwt-04 bad directory alone yields exactly 21 JWT-04 findings", async () => {
	...
	it("CS-INT-37 verify-wrong-key-algorithm.ts in jwt-03 good stays clean with nineteen rules", async () => {
	...
	it("CS-INT-38 jwt-03 bad directory hits JWT-03 JWT-04 and JWT-05 rule ids", async () => {
		const result = await scan({ paths: [jwt03BadDir], cwd: rootDir });
		const ruleIds = new Set(result.findings.map((f) => f.ruleId));
	...
	it("CS-INT-39 hash-01 good and jwt-03 bad yields JWT-03 JWT-04 and JWT-05", async () => {
		const result = await scan({
